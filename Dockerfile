...
RUN wget --progress=dot:giga https://dl.google.com/android/android-sdk_r24.4.1-linux.tgz && mv android-sdk_r24.4.1-linux.tgz /opt/ && cd /opt && tar xzvf ./android-sdk_r24.4.1-linux.tgz && rm -rf /opt/android-sdk_r24.4.1-linux.tgz && apt-get install gcc-multilib -y && apt-get autoclean -y && apt-get autoremove -y
...
# RUN echo y | android update sdk --no-ui --all --filter build-tools-30.0.0
